...
class Part2spiderSpider(scrapy.Spider):
    ...
    start_urls = ["https://simplyhired.com/search?q=&l=10006"]
...
#headers={"User-Agent": self.user_agent_list[random.randint(0,len(self.user_agent_list)-1)]}
    ...
    def parse(self, response):
        ...
        for  title in job_title.range(100):
            yield{
                'Job Page Url' : job_title.css('h3 a').attrib['href'],
                #'jk' :
                'Company Name' : job_title.css('span.homepageSerpJobCompany').get(),
                'Location' : job_title.css('span.homepageSerpJobLocation').get(),
                'Salary': job_title.css('.chakra-text css-1ejkpji').get(),
                'Full Job Description' : job_title.css('div.viewJobBodyJobFullDescriptionContent').get(),
                'Posting Date' : job_title.css('.chakra-text css-1f029gk').get(),
